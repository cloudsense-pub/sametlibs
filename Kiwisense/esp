...
-- Fallback Fonts helper to prevent crashes if Drawing.Fonts is nil or if integer/enum type mismatch occurs
local function getFont(val)
	if Drawing.Fonts then
		if type(val) == "string" and Drawing.Fonts[val] then
			return Drawing.Fonts[val]
		elseif type(val) == "number" then
			local names = { [0] = "UI", [1] = "System", [2] = "Plex", [3] = "Monospace" }
			local name = names[val]
			if name and Drawing.Fonts[name] then
				return Drawing.Fonts[name]
			end
		end
	end
...
function EspObject:Render()
	...
	if visible.healthText.Visible then
		...
		healthText.Font = getFont(interface.sharedSettings.textFont);
		...
		healthText.Position = lerp2(barTo, barFrom, self.health/self.maxHealth) - getTextBounds(healthText)*0.5 - HEALTH_TEXT_OFFSET;
	end
	...
	if visible.name.Visible then
		...
		name.Font = getFont(interface.sharedSettings.textFont);
		...
		name.Position = (corners.topLeft + corners.topRight)*0.5 - Vector2.yAxis*getTextBounds(name).Y - NAME_OFFSET;
	end
	...
	if visible.distance.Visible then
		...
		distance.Font = getFont(interface.sharedSettings.textFont);
	...
	if visible.weapon.Visible then
		...
		weapon.Font = getFont(interface.sharedSettings.textFont);
		...
		weapon.Position =
			...
			(visible.distance.Visible and DISTANCE_OFFSET + Vector2.yAxis*getTextBounds(visible.distance).Y or Vector2.zero);
	end
...
function InstanceObject:Render()
	...
	if text.Visible then
		...
		text.Font = getFont(options.textFont);
